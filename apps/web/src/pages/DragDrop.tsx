...
function DragDrop() {
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    // オプションをスロットへ
    if (
      active.data.current?.type === 'option' &&
...
const Slot: React.FC<{ itemId: string; selected?: Option }> = ({ itemId, selected }) => {
  const { isOver, setNodeRef } = useDroppable({
    id: `slot-${itemId}`,
    data: { type: 'slot', itemId },
  });
  return (
    <div
      ref={setNodeRef}
      className="flex h-12 w-64 items-center justify-center rounded border border-dashed border-slate-400 bg-slate-100 px-3"
